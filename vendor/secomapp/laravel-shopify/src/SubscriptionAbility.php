...
class SubscriptionAbility
{
    ...
    {
        $usage = $this->subscription->usage->first(function ($value) use ($feature) {
    ...
    public function enabled($feature)
    {
        ...
        // feature is enabled.
        if (in_array(strtoupper($feature_value), config('billing.positive_words')))
            return true;
    ...
    {
        $feature = $this->subscription->plan->features->first(function ($value) use ($feature) {
            return $value->code === $feature;
        });
